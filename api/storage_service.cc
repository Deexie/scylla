...
void set_sstables_loader(http_context& ctx, routes& r, sharded<sstables_loader>& sst_loader) {
    ...
    ss::start_restore.set(r, [&sst_loader] (std::unique_ptr<http::request> req) -> future<json::json_return_type> {
        ...
        auto task_id = co_await sst_loader.local().download_new_sstables_async(keyspace, table, prefix, std::move(sstables), endpoint, bucket);
...
void set_snapshot(http_context& ctx, routes& r, sharded<db::snapshot_ctl>& snap_ctl) {
    ...
    ss::start_backup.set(r, [&snap_ctl] (std::unique_ptr<http::request> req) -> future<json::json_return_type> {
        ...
        auto task_id = co_await ctl.start_backup_async(std::move(endpoint), std::move(bucket), std::move(prefix), std::move(keyspace), std::move(table), std::move(snapshot_name));
        co_return json::json_return_type(fmt::to_string(task_id));
    });
