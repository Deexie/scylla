...
future<> repair::remote_metas::for_each_meta_on_all_shards(std::function<future<>(const tablet_repair_task_meta&)> func) const {
    for (size_t shard = 0; shard < smp::count; ++shard) {
        co_await smp::submit_to(shard, [func, this]() mutable {
            return for_each_local_meta(func);
        });
    }
}
...
future<> repair::remote_metas_builder::allocate_all_shards() {
    return parallel_for_each(smp::all_cpus(), [this](size_t shard) -> future<> {
        return allocate_on_shard(shard);
    });
}

future<> repair::remote_metas_builder::add_on_shard(size_t shard_id, tablet_repair_task_meta meta) {
    ++_remote_metas._metas_count;
    co_await smp::submit_to(shard_id, [meta = std::move(meta), &metas_ptr = _remote_metas._metas_on_shards[shard_id]]() {
        if (!metas_ptr) {
            on_internal_error(rlogger, format("remote_metas_builder::add_on_shard: metas vector isn't created on shard_id={}", shard_id));
        }
        metas_ptr->metas.push_back(meta);
    });
}

repair::remote_metas repair::remote_metas_builder::build() && {
    return std::move(_remote_metas);
}
...
future<> repair_service::repair_tablets(repair_uniq_id rid, sstring keyspace_name, std::vector<sstring> table_names, bool primary_replica_only, dht::token_range_vector ranges_specified, std::vector<sstring> data_centers, std::unordered_set<locator::host_id> hosts, std::unordered_set<locator::host_id> ignore_nodes, std::optional<int> ranges_parallelism) {
    ...
    for (auto& table_name : table_names) {
        ...
        task_metas_builder.allocate_all_shards().get();
        size_t nr = 0;
        for (auto& m : metas) {
            nr++;
            ...
            for (auto& r : intersection_ranges) {
                ...
                co_await task_metas_builder.add_on_shard(master_shard_id, tablet_repair_task_meta{keyspace_name, table_name, tid, master_shard_id, r, repair_neighbors(nodes, shards), m.replicas, erm});
            }
        }
    }
    auto task = co_await _repair_module->make_and_start_task<repair::tablet_repair_task_impl>({}, rid, keyspace_name, tasks::task_id::create_null_id(), table_names, streaming::stream_reason::repair, std::move(task_metas_builder).build(), ranges_parallelism, service::default_session_id);
}
...
future<gc_clock::time_point> repair_service::repair_tablet(gms::gossip_address_map& addr_map, locator::tablet_metadata_guard& guard, locator::global_tablet_id gid, tasks::task_info global_tablet_repair_task_info, service::frozen_topology_guard topo_guard, std::optional<locator::tablet_replica_set> rebuild_replicas) {
    ...
    repair::remote_metas_builder task_metas_builder;
    co_await task_metas_builder.allocate_on_shard(*master_shard_id);
    auto ranges_parallelism = std::nullopt;
    ...
    auto task_impl_ptr = seastar::make_shared<repair::tablet_repair_task_impl>(_repair_module, id, keyspace_name, global_tablet_repair_task_info.id, table_names, streaming::stream_reason::repair, std::move(task_metas_builder).build(), ranges_parallelism, topo_guard, rebuild_replicas.has_value());
...
future<> repair::tablet_repair_task_impl::run() {
    ...
    co_await m->run(id, [this, &rs, id] () mutable {
        ...
        _metas.for_each_meta_on_all_shards([&participants, &table_ids] (const tablet_repair_task_meta& meta) -> future<> {
