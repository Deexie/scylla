...
future<> run_table_tasks(replica::database& db, std::vector<table_tasks_info> table_tasks, seastar::condition_variable& cv, tasks::task_manager::task_ptr& current_task, bool sort) {
    ...
    while (!table_tasks.empty()) {
        try {
            if (table_tasks.size() == 1) {
                co_await utils::get_local_injector().inject("compaction_run_table_tasks", utils::wait_for_message(120s));
            }
            if (sort) {
...
future<uint64_t> compaction_task_impl::get_table_task_workload(replica::database& db, const table_info& ti) const {
    ...
    co_await run_on_table("find_compaction_task_progress", db, _status.keyspace, ti, [&bytes] (replica::table& t) -> future<> {
        co_await t.parallel_foreach_compaction_group([&bytes, &t] (replica::compaction_group& cg) -> future<> {
            bytes += std::ranges::fold_left(t.get_compaction_manager().get_candidates(cg.as_table_state(), *cg.as_table_state().main_sstable_set().all()) | std::views::transform([] (auto& sst) { return sst->data_size(); }), int64_t(0), std::plus{});
...
future<std::optional<double>> scrub_sstables_compaction_task_impl::expected_total_workload() const {
    auto table_infos = _column_families | std::views::transform([this] (const auto& cf) {
        return table_info{
            .name = _status.table,
            .id = _db.local().find_uuid(_status.keyspace, _status.table)
        };
