...
async def check_top_level_task(tm: TaskManagerClient, server: ServerInfo, module_name: str, expected_state: str, expected_type: str) -> None:
...
async def check_bootstrap(manager: ManagerClient, tm: TaskManagerClient, servers: list[ServerInfo], module_name: str) -> list[ServerInfo]:
    ...
    check_top_level_task(tm, bootstrapped_server, module_name, "done", "bootstrap")

    servers.append(bootstrapped_server)
    return servers

async def check_replace(manager: ManagerClient, tm: TaskManagerClient, servers: list[ServerInfo], module_name: str) -> list[ServerInfo]:
    ...
    check_top_level_task(tm, replacing_server, module_name, "done", "bootstrap")
    
    servers = servers[1:] + [replacing_server]
    return servers

async def check_rebuild(manager: ManagerClient, tm: TaskManagerClient, servers: list[ServerInfo], module_name: str) -> list[ServerInfo]:
    ...
    check_top_level_task(tm, rebuilt_server, module_name, "done", "rebuild")
    
    return servers

async def check_remove_node(manager: ManagerClient, tm: TaskManagerClient, servers: list[ServerInfo], module_name: str) -> list[ServerInfo]:
    assert servers, "No servers available"
    ...
    check_top_level_task(tm, removing_server, module_name, "done", "removenode")
    
    return servers[1:]

async def check_decommission(manager: ManagerClient, tm: TaskManagerClient, servers: list[ServerInfo], module_name: str) -> list[ServerInfo]:
    ...
    handler = await inject_error_one_shot(manager.api, decommissioned_server.ip_addr, "node_ops_start_decommission_task_impl_run")
    logger.info(f"Decommissioning node {decommissioned_server}")
    await asyncio.gather(*(manager.decommission_node(decommissioned_server.server_id), _check_top_level_decommission_task(decommissioned_server, handler)))
...
async def test_node_ops_tasks(manager: ManagerClient):
    ...
    servers = await manager.running_servers()   # Get rid of bootstrap tasks.
    assert module_name in await tm.list_modules(servers[0].ip_addr), "node_ops module wasn't registered"
    await asyncio.gather(*([]))
    [await tm.drain_module_tasks(s.ip_addr, module_name) for s in servers]
