...
async def test_repair_fails_and_recovers_when_flush_fails(manager: ScyllaClusterManager):
    ...
    flush time and the repair ends as failed without recording a repair time,
    as it did when the repair flushed itself. Once the node flushes again the
    rescheduled repair completes.
    """
    injection = "repair_flush_hints_batchlog_handler_bm_uninitialized"
    servers, cql, hosts, ks, table_id = await create_table_insert_data_for_repair(manager, tablets=4, disable_flush_cache_time=True)
    logs = await open_logs(manager, servers)
    ...
    try:
        ...
        logger.info(f"Repair did not complete while the flush fails, as expected: {e}")
...
async def test_no_flush_for_table_without_repair_tombstone_gc(manager: ScyllaClusterManager):
    ...
    cql = manager.get_cql()
    ks = await create_new_test_keyspace(cql, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3} AND tablets = {'initial': 8};")
    await cql.run_async(f"CREATE TABLE {ks}.test (pk int PRIMARY KEY, c int) WITH tombstone_gc = {{'mode': 'timeout'}};")
...
async def test_repair_flushes_itself_for_request_without_flush_mode(manager: ScyllaClusterManager):
    ...
    assert self_flushes == 4 * len(servers)
    times = await load_tablet_repair_time(cql, hosts[0:1], table_id)
