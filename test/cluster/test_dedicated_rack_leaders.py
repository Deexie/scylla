...
async def terms(manager: ManagerClient, host) -> dict[str, int]:
...
async def test_dedicated_rack_wins_elections(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager,
        ...
        for round_no in range(ROUNDS):
            ...
            without = await leaders(manager, observer.ip_addr, groups)
            logger.info(f"round {round_no}: with the dedicated rack down, "
                        f"{sum(1 for g in groups if without[g] == dedicated_host_id)} groups "
                        f"still report it as the leader")
...
async def test_dedicated_rack_regains_leadership(manager: ManagerClient):
    ...
    async with new_test_keyspace(manager,
        ...
        async def writer():
            ...
            while writing:
                try:
                    ...
                    pass  # a leader change fails a write; keep going
                pk += 1
                await asyncio.sleep(0.02)

        writer_task = asyncio.create_task(writer())

        # The preferred replica holds the first slot but refuses to campaign, so every group
        # ends up led from another rack while it is up and healthy.
        ...
        # Handing it to whoever is up to date instead would move it to another replica, which
        # would hand it on in turn, and the group would keep churning.
        settled = await leaders(manager, observer.ip_addr, groups)
        await asyncio.sleep(3 * PLACEMENT_INTERVAL)
        ...
        assert not moved, f"the leadership moved while the preferred replica could not take over: {moved}"

        # Let it win again. Nothing announces that - it knows the leaders and stays a
        # follower, and their own state doesn't change - so the leadership only comes back
        # because they reconsider it themselves.
