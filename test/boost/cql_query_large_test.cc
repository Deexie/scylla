...
SEASTAR_THREAD_TEST_CASE(test_large_data) {
    ...
    do_with_cql_env_thread([](cql_test_env& e) {
        ...
        flush(e);
        e.db().invoke_on_all([] (replica::database& dbi) {
            return dbi.parallel_for_each_table([&dbi] (table_id id, lw_shared_ptr<replica::table> table) {
                return dbi.get_compaction_manager().perform_major_compaction((table)->as_table_state());
            });
        }).get();
