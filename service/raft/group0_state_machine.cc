...
future<> group0_state_machine::merge_and_apply(group0_state_machine_merger& merger) {
    ...
    [&] (mixed_change& chng) -> future<> {
        ...
        bool has_topology_mutations = false;
        ...
        for (auto& cm : chng.mutations) {
            ...
            if (_sp.data_dictionary().find_schema(tid)->ks_name() == db::schema_tables::v3::NAME) {
                ...
                has_topology_mutations = true;
            }
        }

        if (has_topology_mutations) {
...
static void convert_to_mixed_change(group0_command& cmd) {
    std::visit(overloaded_functor{
        [&cmd](schema_change& chng) {
            cmd.change = mixed_change{std::move(chng.mutations)};
        },
        [](broadcast_table_query&) {
            // leave as-is
        },
        [&cmd](topology_change& chng) {
            cmd.change = mixed_change{std::move(chng.mutations)};
        },
        [&cmd](write_mutations& chng) {
            cmd.change = mixed_change{std::move(chng.mutations)};
        },
        [](mixed_change&) {
            // already mixed_change
        },
    }, cmd.change);
}
