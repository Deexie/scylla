...
class topology_coordinator : public endpoint_lifecycle_subscriber
    ...
    future<std::vector<std::pair<sstring, size_t>>> get_keyspaces_that_require_auto_rf_change(const group0_guard& guard) {
    ...
    future<std::unordered_map<sstring, std::set<sstring>>> get_racks_for_auto_rf_change() {
    ...
    future<std::optional<group0_guard>> maybe_schedule_auto_rf_change(group0_guard guard) {
        ...
        auto auto_rf_keyspaces = co_await get_keyspaces_that_require_auto_rf_change(guard);
        if (auto_rf_keyspaces.empty()) {
            rtlogger.debug("No keyspaces require auto RF change");
            co_return std::move(guard);
        }

        std::unordered_map<sstring, std::set<sstring>> allowed_racks = co_await get_racks_for_auto_rf_change();
