...
class topology_coordinator : public endpoint_lifecycle_subscriber
    ...
    void generate_replication_factor_update(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, const replication_update_info& replication_update) {
        ...
        auto current_replication_factor = ks.metadata()->strategy_options();
        auto new_replication_factor = replication_update.new_replication | std::views::filter([] (const auto& r) { return !r.second.empty(); }) | std::ranges::to<cql3::statements::property_definitions::extended_map_type>();
        new_replication_factor.merge(current_replication_factor);

        cql3::statements::ks_prop_defs new_ks_props{cql3::statements::to_flattened_map(current_replication_factor | std::views::filter([] (const auto& ru) {
            return !std::get<locator::rack_list>(ru.second).empty();
        }) | std::ranges::to<cql3::statements::property_definitions::extended_map_type>())};
        new_ks_props.validate();
        auto ks_md = new_ks_props.as_ks_metadata_update(ks.metadata(), get_token_metadata(), _db.features(), _db.get_config());
