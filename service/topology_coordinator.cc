...
class topology_coordinator : public endpoint_lifecycle_subscriber {
    ...
    void generate_rf_change_update(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, planned_replica tablet_replica, removes_replica removes_replica) {
    ...
    void generate_rf_change_request_finish(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, std::optional<sstring> error = std::nullopt) {
    ...
    void generate_keyspace_schema_update(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, lw_shared_ptr<keyspace_metadata> ks_md) {
    ...
    future<> generate_rf_change_updates(utils::chunked_vector<canonical_mutation>& out, const group0_guard& guard, keyspace_rf_change_plan rf_change_plan) {
            ...
            case service::keyspace_rf_change_state::new_step: {
                ...
                if (rf_change_plan.removes_replica) {
                    if (new_ongoing_rf_change_data.rollback) {
                        if (rf_change_plan.replica != new_ongoing_rf_change_data.added_dc_racks.back()) {
                            on_internal_error(rtlogger, fmt::format("Invalid rf change plan: rollback replica (dc = {}; rack = {}) does not match the last added replica (dc = {}; rack = {})",
                                rf_change_plan.replica.dc, rf_change_plan.replica.rack, new_ongoing_rf_change_data.added_dc_racks.back().dc, new_ongoing_rf_change_data.added_dc_racks.back().rack));
                        }
                        new_ongoing_rf_change_data.added_dc_racks.pop_back();
                    }
                    new_ongoing_rf_change_data.removed_dc_racks.push_back(rf_change_plan.replica);
                } else {
                    new_ongoing_rf_change_data.added_dc_racks.push_back(rf_change_plan.replica);
                }
                out.emplace_back(
                        topology_mutation_builder(guard.write_timestamp())
    ...
    future<> handle_tablet_migration(group0_guard guard, bool drain) {
        ...
        if (!preempt) {
            auto plan = co_await _tablet_allocator.balance_tablets(get_token_metadata_ptr(), _topo_sm._topology.ongoing_rf_change_data, {}, get_dead_nodes());
    ...
    future<> handle_tablet_resize_finalization(group0_guard g) {
        ...
        auto tm = get_token_metadata_ptr();
        auto plan = co_await _tablet_allocator.balance_tablets(tm, _topo_sm._topology.ongoing_rf_change_data, {}, get_dead_nodes());
