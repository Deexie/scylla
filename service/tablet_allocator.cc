...
public:
    ...
    // Checks if we need to continue previously started removal/addition of replicas.
    // Returns rf_change_step_data .
    future<std::optional<rf_change_step_data>> get_ongoing_step() {
        ...
        for (auto&& [table, tables] : _tm->tablets().all_table_groups()) {
            ...
            for (const tablet_info& info : tmap.tablets()) {
                if (!_ongoing_rf_change_data->removed_dc_racks.empty()) {
                    endpoint_dc_rack dc_rack = _ongoing_rf_change_data->removed_dc_racks.back();
                    // Replica in dc_rack hasn't been removed yet.
                    if (std::any_of(info.replicas.cbegin(), info.replicas.cend(), [&topo, &dc_rack] (const auto& replica) {
                        const auto& location = topo.get_location(replica.host);
                        return location == dc_rack;
                    })) {
                ...
                } else if (!_ongoing_rf_change_data->added_dc_racks.empty()) {
                    endpoint_dc_rack dc_rack = _ongoing_rf_change_data->removed_dc_racks.back();
                    // Replica hasn't been added to dc_rack yet.
                    if (std::none_of(info.replicas.cbegin(), info.replicas.cend(), [&topo, &dc_rack] (const auto& replica) {
                        const auto& location = topo.get_location(replica.host);
                        return location == dc_rack;
    ...
    future<std::optional<rf_change_step_data>> get_new_step() {
        if (_ongoing_rf_change_data->rollback) {
            if (!_ongoing_rf_change_data->added_dc_racks.empty()) {
                co_return std::nullopt;
            }
            co_return _ongoing_rf_change_data->added_dc_racks.back();
        }
    ...
    future<keyspace_rf_change_plan> make_rf_change_plan(const migration_plan& mplan) {
        ...
        } else if (ongoing_rf_change_data.rollback && ongoing_rf_change_data.added_dc_racks.empty()) {
