...
struct tablet_rack_list_colocation_plan {
    utils::chunked_vector<tablet_migration_info> _colocations;
    utils::UUID _request_to_resume;

    const utils::chunked_vector<tablet_migration_info>& colocations() const noexcept{
        return _colocations;
    }
    ...
    size_t size() const { return _colocations.size() + (_request_to_resume ? 1 : 0); };

    void merge(tablet_rack_list_colocation_plan&& other) {
        for (auto& r : other._colocations) {
            _colocations.push_back(r);
        }
        _request_to_resume = _request_to_resume ? _request_to_resume : other._request_to_resume;
    }

    void add_colocation(const tablet_migration_info& info) {
        _colocations.push_back(info);
    }
